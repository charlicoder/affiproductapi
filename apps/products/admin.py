...
class ProductAdmin(admin.ModelAdmin):
    list_display = [
        'title_short', 'product_asin', 'brand', 'category', 'sub_category', 
        'price', 'rating', 'featured_image_preview', 'featured', 'published', 
        'active', 'created_at'
    ]
    list_filter = [
        'brand', 'category', 'sub_category', 'platform', 'featured', 
        'published', 'active', 'created_at', 'updated_at'
    ]
    search_fields = [
        'title', 'product_asin', 'description', 'brand__name', 
    ...
    fieldsets = (
        ...
        ('Pricing', {
            'fields': ('price', 'regular_price', 'cost_savings', 'discount_percent', 'savings_percent'),
            'classes': ('collapse',)
        }),
        ('Product Details', {
            'fields': ('rating', 'shipping', 'returns'),
            'classes': ('collapse',)
        }),
        ('Status', {
            'fields': ('featured', 'published', 'active')
        }),
    )
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "category":
            ...
            kwargs["queryset"] = SubCategory.objects.filter(active=True).select_related('category')
    ...
    actions = ['make_featured', 'remove_featured', 'publish', 'unpublish', 'activate', 'deactivate']
...
class ProductImageAdmin(admin.ModelAdmin):
    list_display = ['product_title', 'image_preview', 'is_featured', 'order', 'created_at']
    list_filter = ['is_featured', 'created_at']
    search_fields = ['product__title', 'alt_text']
    
    def product_title(self, obj):
        return obj.product.title[:50] + "..." if len(obj.product.title) > 50 else obj.product.title
...
class ProductVideoAdmin(admin.ModelAdmin):
    list_display = ['product_title', 'title', 'video_preview', 'is_featured', 'order', 'created_at']
    list_filter = ['is_featured', 'created_at']
