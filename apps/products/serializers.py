...
class ProductListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "platform_name",
            "featured_image",
            "images",  # ✅ add this to the fields list
            "tags",
            "featured",
            "created_at",
            "updated_at",
...
class ProductCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for i, image_data in enumerate(images_data):
            ProductImage.objects.create(product=product, order=i, **image_data)

        # Add videos
        for i, video_data in enumerate(videos_data):
            ProductVideo.objects.create(product=product, order=i, **video_data)
    ...
    def update(self, instance, validated_data):
        ...
        if images_data is not None:
            ...
            for i, image_data in enumerate(images_data):
                ProductImage.objects.create(product=instance, order=i, **image_data)

        # Update videos if provided
        if videos_data is not None:
            instance.videos.all().delete()
            for i, video_data in enumerate(videos_data):
                ProductVideo.objects.create(product=instance, order=i, **video_data)
